...
export function svd_jac_2sided_blocked(A)
{
  ...
  const block = (i,j) => BB*(n*Math.floor(i/B) + Math.floor(j/B)) + B*(i%B) + (j%B);
                                         ...
                                         sv_off += N )
  {
   ...
   //
    ...
    {        finished = true;
      for( let C=0; C <  n; C++ )
      for( let R=0; R <= C; R++ )
      for( let c=0; c <  B; c++ ) { const rr = C===R ? c : B;
      for( let r=0; r < rr; r++ )
      {
        const p = B*R + r,
              q = B*C + c;

        const S_pp = S[BB*(n*R+R) + B*r+r],
              S_pq = S[BB*(n*R+C) + B*r+c],
              S_qp = S[BB*(n*C+R) + B*c+r],
              S_qq = S[BB*(n*C+C) + B*c+c];
