...
export const powell_badscale = x =>
{
  x = asarray(x)
...
powell_badscale.nIn =
powell_badscale.nOut= 2;


powell_badscale.minima_global=
powell_badscale.roots        = function(){
  const x1 = 0.0000109815932969981745569,
...
powell_badscale.minima = function(){
  const x = root1d_bisect(
//    x => 1e4*(1e4*x*x - 1)*x - 2*Math.exp(-2*x) + 1.0001*Math.exp(-x),
//    x => 1e4*(1e4*x*x - 1)*x - Math.exp(-x) * (2*Math.exp(-x) - 1.0001),
    x => 1e4*(1e4*x*x - 1)*x + Math.exp(-x) * (0.0001 - Math.expm1(-x) - Math.exp(-x)),
    -0.02,-0.0005
  )
  return [
    ...powell_badscale.minima_global,
...
powell_badscale.hess = x =>
{
  ...
  {        H_off -= 4;
    ...
    H[H_off+1] =
    H[H_off+2] = 2 * ( f1*1e4 + 1e8*x1*x2 + Math.exp(-x1-x2) );
