...
//  if( p.ndim !== 1 ) throw new Error('fit_lm_gen(x,y, fg, p0): p0.ndim must be 1.');
//
//  if( accept     < 0         )    console.warn('fit_lm_gen(x,y, fg, p0, opt): opt.improvement.accept should not be negative.');
//  if( accept     > expectLow ) throw new Error('fit_lm_gen(x,y, fg, p0, opt): opt.improvement.accept must be less than opt.improvement.expectLow.');
//  if( expectHigh < expectLow ) throw new Error('fit_lm_gen(x,y, fg, p0, opt): opt.improvement.expectLow must be less than opt.improvement.expectHigh.');
//  if( expectHigh > 1         )    console.warn('fit_lm_gen(x,y, fg, p0, opt): opt.improvement.expectHigh should not be greater than 1.');
//
//  if( ! (0 <= lambda0 && lambda0 < Infinity) )
//    throw new Error(`fit_lm_gen(x,y, fg, p0, opt): Invalid opt.lambda0=${lambda0}.`);
//
//  if( lambdaFactor instanceof Array )
//  {
//    if(  lambdaFactor.length !== 2  ) throw new Error(`fit_lm_gen(x,y, fg, p0, opt): opt.lambdaFactor must contain two numbers if Array.`);
//    const [lo,hi] = lambdaFactor;
//    if( ! (0 < lo && lo < 1       ) ) throw new Error(`fit_lm_gen(x,y, fg, p0, opt): opt.lambdaFactor[0] must be in (0,1).`);
//    if( ! (1 < hi && hi < Infinity) ) throw new Error(`fit_lm_gen(x,y, fg, p0, opt): opt.lambdaFactor[1] must be in (1,∞)`);
//  }
//  else if( ! (1 <= lambdaFactor && lambdaFactor < Infinity) )
//    throw new Error(`fit_lm_gen(x,y, fg, p0, opt): Invalid opt.lambdaFactor=${lambda0}.`);
//
//  const [P] = p.shape,
//      [M,N] = x.shape,
//    p_shape = p.shape,
//    x_shape = Int32Array.of(N),
//    r_shape = Int32Array.of(M),
//    J_shape = Int32Array.of(M,P),
...
//  {
//    const fgp = fg(
//      new NDArray(p_shape, p.slice()) // <- protection copy
//    );
...
//      for( let j=P; j-- > 0; )
//      {
//        G[j]    += g[j] * R[i] / M; // <- TODO check underflow ?
//        J[P*i+j] = g[j];
//      }
//    }
//
//    mse = 0.5 * R.reduce((e,x) => e + x*x/M, 0); // <- TODO check underflow ?
//  };
//
//  const compute_dp = (J,R,lambda) => {
//    let dp = rls(
//      new NDArray(J_shape, J.slice()),
//      new NDArray(R_shape, R.slice()),
//      lambda
//    );  dp = asarray(dp);
//    if( dp.ndim     !== 2 ) throw new Error('Assertion failed.');
//    if( dp.shape[0] !== P ) throw new Error('Assertion failed.');
//    if( dp.shape[1] !== 1 ) throw new Error('Assertion failed.');
//    return dp.data;
//  };
//
//  if( lambdaFactor === 1 )
...
//      yield [
//        /*meanSquareErr=*/mse,
//        /*gradient =*/new NDArray(p_shape, G.slice()),
//        /*params   =*/new NDArray(p_shape, p.slice()),
//        /*residuals=*/new NDArray(r_shape, R.slice())
//      ];
//
//      const dp = compute_dp(J,R, lambda0);
//
//      for( let i=P; i-- > 0; )
//        p[i] -= dp[i];
//    }
...
//      for(;;)
//      {
//        const dp = compute_dp(_J,_R, lambda);
//
//        for( let i=P; i-- > 0; )
//          p[i] = _p[i] - dp[i];
//
...
//        for( let j=0; j < P; j++ )       r-= _J[P*i+j] * dp[j];
//            r += _R[i]
//          est += r*r/M;
//        }
//        est *= 0.5;
//
//        if( est > _mse ) throw new Error('Assertion failed.');
//
