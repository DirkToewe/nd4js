...
export function regular_simplex( dtype, N )
{
  if( null == N ) {
    N = dtype;
        dtype = 'float64';
    ...
    if( ! (dtype in ARRAY_TYPES)   ) throw new Error(`regular_simplex(dtype,N): dtype '${dtype}' not supported.`);
  ...
  const V = new ARRAY_TYPES[dtype]( (N+1)*N ),
    shape =          Int32Array.of( (N+1),N );

  // The idea of the algorithm is to "recursively" build an i-simplex from an (i-1)
