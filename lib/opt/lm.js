    ...
    while (1) {
      switch (_context.prev = _context.next) {
        ...
        case 41:
          ...
          X = x0.data, W = new Float64Array(X.length);
        ...
        case 54:
          f = ff.data, J = JJ.data;
          solver.update(ff, JJ); //  let R = r0; // <- TODO maybe there is a better starting value for R0 like r0*solver.scaledNorm(G) order something like that

          R = -r0 * solver.cauchyPointTravel() * solver.scaledNorm(G);
          err_now = 0;
        ...
        case 124:
          ...
          rScale = function rScale() {
            return solver.scaledNorm(G);
          }; // <- TODO: examine scaling alternatives


          predict = err_now - err_predict, actual = err_now - err_next;

          if (!(actual > predict * expectGainMax || same_x)) {
            _context.next = 133;
            break;
          }

          R = Math.min(rMax * rScale(), Math.max((0, _float64_utils.nextUp)(R), R * grow)); // PREDICTION GREAT -> INCREASE TRUST RADIUS TODO: Consider using [1] Algorithm (7.1) (d)
