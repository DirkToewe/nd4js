...
exports.fit_param_lin = fit_param_lin;
...
function fit_param_lin(x, y, regularization, funcs) {
  if (funcs == null) {
    funcs = regularization;
    regularization = 0;
  } else if (null == regularization) regularization = 0;else if (!(0 <= regularization && regularization < Infinity)) throw new Error("fit_param_lin(x,y, regularization, funcs): Invalid regularization: ".concat(regularization, "."));

  funcs = (0, _toConsumableArray2["default"])(funcs);
  x = (0, _nd_array.asarray)('float64', x);
  y = (0, _nd_array.asarray)('float64', y);
  if (y.ndim !== 1) throw new Error('fit_param_lin(x,y, (regularization,) funcs): y.ndim must be 1.');
  var M = y.shape[0],
      N = funcs.length;
  var x_ndim = x.ndim;
  if (x.ndim !== 1 && x.ndim !== 2) throw new Error('fit_param_lin(x,y, (regularization,) funcs): x.ndim must be 1 or 2.');
  if (x.shape[0] !== y.shape[0]) throw new Error('fit_param_lin(x,y, (regularization,) funcs): x.shape[0] and y.shape[0] must equal.');
