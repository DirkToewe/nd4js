  ...
  for (var _i = 0; _i < N; _i++) {
    ...
    if (_i < N - 2) {
      ...
      var _norm = NORM.resultIncl(U[ii + 1]) * (U[ii + 1] > 0 ? -1 : +1),
          div = NORM.resultIncl(U[ii + 1] -= _norm);

      for (var _j2 = _i; ++_j2 < N;) {
        U[U_off + N * _i + _j2] /= div;
...
function _bidiag_decomp_horiz(M, N, U, U_off, B, B_off, V, V_off) {
  ...
  for (var _i7 = 0; _i7 < M && _i7 < N - 1; _i7++) {
    // ELIMINATE ELEMENTS BELOW (i,i) USING GIVENS
    ...
    var norm = NORM.resultIncl(V[above + (_i7 + 1)]) * (V[above + (_i7 + 1)] > 0 ? -1 : +1),
        denom = NORM.resultIncl(V[above + (_i7 + 1)] -= norm);

    for (var _j12 = _i7; ++_j12 < N;) {
      V[above + _j12] /= denom;
    }

    if (0 === NORM.max) continue; // apply householder to right of V
...
function bidiag_decomp(A) {
  ...
  var _ref = function () {
    if (M === N) {
      ...
      var _B2 = new DTypeArray(len * M * M),
          _V2 = new DTypeArray(len * M * M);

      for (var U_off = 0, BV_off = 0; BV_off < _B2.length; U_off += N * M, BV_off += M * M) {
        _bidiag_decomp_vert(N, M, _U2, U_off, _B2, _V2, BV_off);
      }

      return [_U2, _B2, _V2];
    } else {
      /*N < M*/
      var _V3 = new DTypeArray(len * (N + 1) * M);

      A = A.data;

      for (var j = 0, i = M; i < _V3.length; i += M) {
        for (var _I = i + N * M; i < _I; j++, i++) {
          _V3[i] = A[j];
        }
      }
      ...
      var _U3 = new DTypeArray(len * N * N),
          _B3 = new DTypeArray(len * N * (N + 1));

      for (var _U_off = 0, B_off = 0, V_off = 0; B_off < _B3.length; _U_off += N * N, B_off += N * (N + 1), V_off += (N + 1) * M) {
        _bidiag_decomp_horiz(N, M, _U3, _U_off, _B3, B_off, _V3, V_off);
      }
